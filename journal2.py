...
speeds_to_simulate = [25] #list(range(20, 41, 5))  # 20, 25, 30, 35, 40
...
for speed_sim in speeds_to_simulate:
    ...
    total_packets_to_send = 300 # Send only 300 packets
    packets_per_second = 200 # Increased packet sending rate
    sent_packet_count = 0
...
def run_scenario(
    ...
    # Setup
    config = Configure(num_nodes=num_nodes, area_dim=area_dim)
    opportunistic = Opportunistic(config)
    mobility = MobilityModel(config, space_dim=area_dim, speed_range=(speed, speed))
    source = config.nodes[0]
    destination = config.nodes[-1]
    ...
    sent_packet_count = 0
    # For routing overhead: count all control packets (hello + distance updates)
    routing_overhead_count = 0

    start_time = time.time()
    for timestep in range(simulation_steps):
        mobility.update_positions()
        opportunistic.forwarder()
        # Count hello packets as routing overhead
        routing_overhead_count += num_nodes * (num_nodes - 1)  # Each node sends hello to every other node

        # Simulate interference by randomly dropping packets in queue
        if interference:
            for node in config.nodes:
                if node.opp_packet_queue and random.random() < 0.05:
                    node.opp_packet_queue.clear()

        # Heavy traffic: send more packets per step
        if heavy_traffic:
            packets_this_step = packets_per_second * 2
        else:
            packets_this_step = packets_per_second

        if sent_packet_count < total_packets_to_send:
            to_send = min(packets_this_step, total_packets_to_send - sent_packet_count)
            for _ in range(to_send):
                source.send_packet(destination.ip_address)
            sent_packet_count += to_send

        for node in config.nodes:
            node.update_position_log()
            node.process_queue()
            node.process_distance_updates()
            # Count distance update packets as routing overhead
            routing_overhead_count += len(node.distance_update_queue)
    ...
    if Node.delivered_packets:
        for packet_info in Node.delivered_packets:
            if 'creation_timestamp' in packet_info and 'delivery_timestamp' in packet_info:
                delay = (packet_info['delivery_timestamp'] - packet_info['creation_timestamp']).total_seconds() * 1000
                delays.append(delay)
    avg_delay_ms = np.mean(delays) if delays else 0.0

    # Routing overhead (pkts/s)
    routing_overhead = routing_overhead_count / sim_duration if sim_duration > 0 else 0.0

    return {
        "Scenario": scenario_name,
        "PDR (%)": round(pdr, 2),
        "E2E Delay (ms)": round(avg_delay_ms, 2),
        "Routing Overhead (pkts/s)": round(routing_overhead, 2)
    }

# --- Scenario Definitions ---
scenarios = [
    {
        "name": "LOW MOBILITY, SPARSE NETWORK",
        "num_nodes": 20,
        "area_dim": 2000,
        "speed": 10,
        "total_packets_to_send": 100,
    ...
    {
        "name": "HIGH MOBILITY, DENSE NETWORK",
        "num_nodes": 100,
        "area_dim": 800,
        "speed": 50,
        "total_packets_to_send": 500,
    ...
    {
        "name": "HIGH MOBILITY, SPARSE NETWORK WITH INTERFERENCE",
        "num_nodes": 25,
        "area_dim": 2000,
        "speed": 40,
        "total_packets_to_send": 200,
    ...
    {
        "name": "MODERATE MOBILITY, DENSE NETWORK WITH HEAVY TRAFFIC",
        "num_nodes": 80,
        "area_dim": 800,
        "speed": 15,
...
for sc in scenarios:
    ...
    result = run_scenario(
        ...
        area_dim=sc["area_dim"],
        speed=sc["speed"],
        total_packets_to_send=sc["total_packets_to_send"],
        packets_per_second=sc["packets_per_second"],
        simulation_steps=sc["simulation_steps"],
        interference=sc.get("interference", False),
        heavy_traffic=sc.get("heavy_traffic", False)
    )
    scenario_results.append(result)

# --- Print Results Table ---
print("\n=== Scenario Results ===")
header = "{:<45} {:>10} {:>18} {:>25}".format("Scenario", "PDR (%)", "E2E Delay (ms)", "Routing Overhead (pkts/s)")
print(header)
print("-" * len(header))
for res in scenario_results:
    print("{:<45} {:>10} {:>18} {:>25}".format(
        res["Scenario"], res["PDR (%)"], res["E2E Delay (ms)"], res["Routing Overhead (pkts/s)"]
    ))

# --- Plot Results ---
import matplotlib.pyplot as plt

labels = [r["Scenario"] for r in scenario_results]
pdrs = [r["PDR (%)"] for r in scenario_results]
delays = [r["E2E Delay (ms)"] for r in scenario_results]
overheads = [r["Routing Overhead (pkts/s)"] for r in scenario_results]

x = np.arange(len(labels))
width = 0.5

plt.figure(figsize=(12, 5))
plt.bar(x, pdrs, width, color='skyblue')
plt.xticks(x, labels, rotation=20, ha='right')
plt.ylabel("PDR (%)")
plt.title("Packet Delivery Ratio by Scenario")
plt.tight_layout()
plt.show()

plt.figure(figsize=(12, 5))
plt.bar(x, delays, width, color='orange')
plt.xticks(x, labels, rotation=20, ha='right')
plt.ylabel("End-to-End Delay (ms)")
plt.title("Average End-to-End Delay by Scenario")
plt.tight_layout()
plt.show()

plt.figure(figsize=(12, 5))
plt.bar(x, overheads, width, color='green')
plt.xticks(x, labels, rotation=20, ha='right')
plt.ylabel("Routing Overhead (pkts/s)")
plt.title("Routing Overhead by Scenario")
plt.tight_layout()
plt.show()
...
def run_single_simulation(seed, num_nodes_sim, speed_sim, total_packets_to_send, packets_per_second):
    random.seed(seed)
    np.random.seed(seed)
    ...
    destination = config.nodes[num_nodes_sim - 1]
    sent_packet_count = 0
    simulation_steps = math.ceil(total_packets_to_send / packets_per_second) + 50
    start_time = time.time()
    for timestep in range(simulation_steps):
        mobility.update_positions()
        opportunistic.forwarder()
    ...
    if Node.delivered_packets:
        for packet_info in Node.delivered_packets:
            if 'creation_timestamp' in packet_info and 'delivery_timestamp' in packet_info:
                ...
                delay = (delivery_time - creation_time).total_seconds()
                end_to_end_delays.append(delay)
    avg_e2e_delay = np.mean(end_to_end_delays) if end_to_end_delays else 0.0
    estimated_average_packet_size_bytes = np.mean(Node.delivered_packet_sizes) if Node.delivered_packet_sizes else 0
    throughput_bps = 0
    if total_packets_delivered > 0 and simulation_duration > 0 and estimated_average_packet_size_bytes > 0:
        total_data_delivered_bytes = total_packets_delivered * estimated_average_packet_size_bytes
        total_data_delivered_bits = total_data_delivered_bytes * 8
...
def run_parallel_simulations(num_runs, num_nodes_sim, speed_sim, total_packets_to_send, packets_per_second):
    seeds = [1000 + i for i in range(num_runs)]
    results = []
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = [
            executor.submit(run_single_simulation, seed, num_nodes_sim, speed_sim, total_packets_to_send, packets_per_second)
            for seed in seeds
        ]
    ...
    for key in results[0]:
        ...
        metrics[key + "_ci95"] = 1.96 * np.std(vals) / np.sqrt(num_runs)
    return metrics
...
if __name__ == "__main__":
    ...
    num_nodes = 100
    speed = 50
    total_packets_to_send = 400
    packets_per_second = 400
    print("\n=== Running Parallel Simulations ===")
    agg_metrics = run_parallel_simulations(
        num_parallel_runs, num_nodes, speed, total_packets_to_send, packets_per_second
    )
    print("Aggregated Metrics (mean, std, 95% CI):")
