...
class TGNNLinkLifetime(nn.Module):
    ...
    def forward(self, x_seq, edge_index_seq):
        ...
        for t in range(self.window):
            ...
            gcn_outs.append(gcn2_out.unsqueeze(0))  # (1, nodes, hidden_dim)
        gcn_outs = torch.cat(gcn_outs, dim=0)  # (window, nodes, hidden_dim)
        # For each node, treat its sequence as input to GRU
        # We'll predict for the first neighbor (since each TGNN is per-node-per-link)
        # For simplicity, assume batch size 1 (per node, per neighbor)
...
class Node:
    def __init__(self, node_id, address, mac_address):
        ...
        self.tgnn_targets = []  # (features, edge_index_seq, target_lifetime)
    ...
    def _calculate_relative_features(self, neighbor_node, current_time):
        ...
        neighbor_pos = np.array([neighbor_node.x, neighbor_node.y, neighbor_node.z])
    ...
    def update_link_state_tracker(self, current_time, current_neighbors_ids, all_nodes):
        ...
        for tracked_neighbor_id in list(self.link_state_tracker.keys()):
            if tracked_neighbor_id not in current_neighbors_ids:
                ...
                actual_link_lifetime = current_time - link_info['start_time']
                # --- Store TGNN training sample if enough history ---
                ...
                if tgnn_hist and len(tgnn_hist) == 5:
                    ...
                    self.tgnn_targets.append((x_seq, edge_index_seq, torch.tensor([actual_link_lifetime], dtype=torch.float32).to(device)))
...
if __name__ =="__main__":
    ...
    for step in range(steps):
        ...
        for src_id, dst_id, is_last_packet in delivered_packets:
            ...
            delay = current_time - sent_time + time_per_step_seconds  # Use actual time difference
        ...
        throughput = (len(delivered_packets) * packet_size_bits) / time_per_step_seconds  # Modified throughput calculation
        ...
        print(f"Step {step}: PDR = {pdr:.2f}, Throughput = {throughput:.2f} bps, Avg Delay = {avg_delay:.2f} s, GNN Loss = {gnn_loss_list[-1]:.4f}, MAB Avg Reward = {mab_avg_reward_list[-1]:.3f}")
        print("="*60)
    ...
    plt.xlabel("Step")
    plt.ylabel("Throughput (bits/second)") # Modified y-axis label
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel("Step")
    plt.ylabel("Delay (seconds)") # Modified y-axis label
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    # --- Optional: TGNN training monitoring ---
    # plt.figure(figsize=(7,4))
    # plt.plot(range(steps), tgnn_loss_list, marker='o', color='brown')
